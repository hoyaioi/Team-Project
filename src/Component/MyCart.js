...
function MyCart() {
  ...
  const plusClick = (cartIdx, itemAmount) => {
    ...
    axios
      .post("http://localhost:8080/cartupdate", cartListDto)
      .then((response) => {
  ...
  const cartDelete = () => {
    ...
    const confirm = window.confirm(
      checkedList.length + "개의 상품 정말 삭제하시겠습니까?"
    );
    if (confirm)
      axios
        .post("http://localhost:8080/cartdelete", checkedList)
        .then((response) => {
          console.log(response);
          alert("삭제성공");
          setData(response.data);
