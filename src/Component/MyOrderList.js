...
function MyOrderList({ memIdx }) {
  ...
  const handlerCancelNow = (orderNum) => {
    if (window.confirm("해당 상품의 주문을 취소하시겠습니까?")) {
      axios
        .put(`http://localhost:8080/mypage/myorderlist/now/${orderNum}`)
        .then((response) => {
  ...
  const handlerCancelPlz = (orderNum) => {
    if (
      ...
      axios
        .put(`http://localhost:8080/mypage/myorderlist/plz/${orderNum}`)
        .then((response) => {
          if (response.status === 200) {
            alert(
              "신청이 완료되었습니다. \n판매자의 승인 후 취소가 완료됩니다."
  ...
  const handlerPurchase = (memIdx, itemName, itemNum, orderNum) => {
    if (
      ...
      axios
        .put(`http://localhost:8080/mypage/myorderlist/purchase/${orderNum}`)
        .then((response) => {
          if (response.status === 200) {
            axios
              .post(
                `http://localhost:8080/mypage/myorderlist/purchase/${orderNum}`,
                {
                  memIdx: memIdx,
                  itemName: itemName,
                  itemNum: itemNum,
                        ...
                        <td className="myorderlist_order_btn_td">
                          ...
                          <div>
                            ...
                            {order.orderStatus === "주문완료" ? (
                              <button
                                type="button"
                                onClick={() => handlerCancelNow(order.orderNum)}
                              >
                                취소요청
                              </button>
                            ) : (
                            ...
                            {order.orderStatus === "상품준비중" ? (
                              <button
                                type="button"
                                onClick={() => handlerCancelPlz(order.orderNum)}
                              >
                                취소요청
                              </button>
                            ) : (
